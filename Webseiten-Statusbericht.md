...
- **Vereinfachter Prouftraining-Input (geplant):** Statt einer langen Mitgliederauswahl in der Anwesenheits-Liste erhält der Trainer ein Formular mit Vorname, Nachname, Geburtsdatum, Handynummer und den letzten 5 Ziffern der KV-Karte. Existiert der Spieler in der Datenbank, wird er direkt als Prouftraining für das aktuelle Team/Tag eingetragen – für alle Aufsteiger-Kombinationen (U7↔U9, U13↔U15, Jugend↔Senioren, Frauen↔Männer). Ist er unbekannt, kann der Trainer ihn entweder als neuen Member im Status `pending` anlegen oder nur als temporären Training-Gast für den Tag erfassen. Regelmäßige Aufsteiger (`extraTeamIds`) erscheinen automatisch in der Suche.
