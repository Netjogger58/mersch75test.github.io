...
- **Vereinfachter Prouftraining-Input (umgesetzt):** Statt einer langen Mitgliederauswahl in der Anwesenheits-Liste erhält der Trainer jetzt ein Suchfeld plus ein Formular mit Vorname, Nachname, Geburtsdatum, Handynummer und den letzten 5 Ziffern der KV-Karte. Existiert der Spieler in der Datenbank, wird er direkt als Prouftraining für das aktuelle Team/Tag eingetragen – für alle Aufsteiger-Kombinationen (U7↔U9, U13↔U15, Jugend↔Senioren, Frauen↔Männer). Ist er unbekannt, kann der Trainer ihn entweder als neuen Member im Status `pending` anlegen oder nur als temporären Training-Gast für den Tag erfassen. Bei der Neuanlage wird die Handynummer mit einem Flag «gehört zu Spieler/Elternteil» erfasst bzw. als «unbekannt» vermerkt. Regelmäßige Aufsteiger (`extraTeamIds`) erscheinen automatisch in der Suche.
- **Warteliste (umgesetzt):** Erfassung mit Name, Geburtsdatum, Telefon, E-Mail, gewünschtem Team, Position und Notiz. Status-Workflow: `wartend` → `zum Probetraining eingeladen` → `aufgenommen` / `abgelehnt`. "Als Mitglied übernehmen" legt ein `pending`-Member mit Team/Telefon an.
- **DSGVO-Tools (umgesetzt):** Consent-Übersicht für Datenverarbeitung, Fotos/Video und Newsletter; Admin/Sekretariat kann Einwilligungen pro Mitglied erfassen/ändern; Datenauszug als JSON (Admin & Selbstbedienung); Löschantrags-Workflow mit Genehmigen/Ablehnen inkl. Begründung.
- **Saison-Archiv Import aus Website (umgesetzt):** Admin-Route `POST /api/admin/archive/import-website` parst die Statistik-HTML-Seiten (`Statistics Men/Women/U15/U13/U11.html`) und archiviert Saison, Team-Bilanz, Spiele und Torschützen in den `archive_*`-Tabellen. Damit liegen die Daten fest in der App und sind unabhängig von FLH.
- **Familienverknüpfung (sichtbar):** Im Mitglied-Detail werden Familienangehörige über den `familyCode` / Haushaltscode dargestellt.
- **Code-Modularisierung gestartet:** `server/routes.ts` und `server/storage.ts` werden aufgelöst; neue Module für Archiv, GDPR und Warteliste.
