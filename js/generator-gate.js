...
(function () {
    ...
    var TTL_MS = 7 * 24 * 60 * 60 * 1000; // 7 Tage
    ...
    function interceptLinks() {
        ...
        for (var i = 0; i < links.length; i++) {
            ...
            a.addEventListener('click', function (ev) {
                if (isUnlocked()) return; // direkt durchlassen
                ev.preventDefault();
    ...
    function handlePageLoad() {
        ...
        if (isGenPage) {
            if (!isUnlocked()) {
                // Direktaufruf ohne Login -> zurueck zur Startseite, Modal dort oeffnen
                window.location.replace('index.html?gen=lock');
            }
            return;
        }

        // Auf normalen Seiten: Links abfangen + ggf. Modal direkt oeffnen
        interceptLinks();
